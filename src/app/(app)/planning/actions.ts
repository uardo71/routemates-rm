...
export async function savePlanAction(cells: PlanCell[]): Promise<{ error?: string }> {
  ...
  // Reject any cell whose week falls outside the assignment's own start/end date window —
  // planning time against a week the assignment doesn't cover was previously allowed to save
  // silently, which is exactly wrong (the assignment's window is meant to be the ground truth
  // for which weeks are valid, same as the Time Entry grid's "pickable" check). Clearing a cell
  // to 0 is always allowed, even outside the window, so a stale value can still be removed.
