...
import { companySlaPolicies } from "@/lib/sla.server";
...
export default async function NewTicketPage({ searchParams }: { searchParams: Promise<{ clientId?: string }> }) {
  ...
  const [clients, projects, users, slaPolicies] = await Promise.all([
    ...
    companySlaPolicies(user.companyId),
  ]);
  ...
  const config: FormConfig = {
    types: cfg.types.map((t) => ({ id: t.id, name: t.name, color: t.color, icon: t.icon, slaApplicable: t.slaApplicable, fields: t.fields.map(pubField) })),
