...
export function TaxesClient({
  ...
  // ---- KPIs ---- (sum amounts converted to the reporting currency; unconvertible rows contribute
  // nothing and are surfaced in the FX warning)
  const base = (r: TaxRow) => r.baseAmount ?? 0;
  ...
  for (const r of rows) byCat.set(r.categoryName, (byCat.get(r.categoryName) ?? 0) + base(r));
  ...
  for (const r of rows) if (monthTotals.has(r.periodMonth)) monthTotals.set(r.periodMonth, monthTotals.get(r.periodMonth)! + base(r));
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <FxWarning excluded={excluded} noun="tax payments" reporting={reportingCurrency} />
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card>
            ...
            <CardContent className="flex justify-center">
              <DonutChart segments={categorySegments} centerLabel={formatMoney(totalAll, reportingCurrency)} centerSublabel="total" size={140} />
            </CardContent>
          </Card>
